...
<html>
  ...
  <body>
    ...
    <p class="meta">Order: ${escapeHtml(order.orderNumber || "-")} | Status: ${escapeHtml(statusConfig.label)} | Created: ${escapeHtml(formatDate(order.dateCreated))}</p>
    ...
    <p class="note">Delivered At: ${escapeHtml(formatDate(order.deliveredAt))} | Completed At: ${escapeHtml(formatDate(order.completedAt))}</p>
  </body>
</html>`;
}
...
export function downloadOrderReceipt(
  ...
  const html = buildOrderReceiptHtml(order, formatAmount);
