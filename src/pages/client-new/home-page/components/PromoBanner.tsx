...
export default function PromoBanner({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        "relative w-full py-4 md:py-6 px-6 md:px-8 rounded-xl overflow-hidden bg-[#F7F7F7] text-foreground h-[80px] md:h-[100px] flex items-center shadow-sm",
        className,
      )}
    >
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-center gap-4">
            <div className={cn("hidden md:flex w-12 h-12 rounded-full items-center justify-center shrink-0", currentItem.iconBg)}>
              {currentItem.icon}
            </div>
            <div className="flex flex-col md:gap-1">
              <h3 className="text-[17px] md:text-xl font-bold text-foreground">{currentItem.title}</h3>
